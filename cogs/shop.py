...
# 2-3: 0.1, 4: 0.2, 5: 0.4, 6: 0.7
        ...
        if "customitem" in post:
            ...
            if len(newitem) > 14:
                ...
                if price > 10 or price < 1:
                    ...
                    embed.set_author(icon_url=ctx.me.avatar_url_as(format='png'), name="Custom Item Creation")
                    await ctx.send(embed=embed)
                    db.market.update_one({"owner": ctx.author.id}, {"$push":{"items": {"name": newitem, "price": price, "stock": 0, "sold": 0}}})
    ...
    async def cookt(self, ctx):
        ...
        if post:
            ...
            resp = await self.bot.wait_for('message', check=nc, timeout=30)
            while bar_int <= 6:
                if done:
                    pass
                else:
    ...
    async def restaurant(self, ctx, *, restaurant=None):
        ...
        if not post:
            ...
            embed.add_field(name=":name_badge: Owner", value=str(self.bot.fetch_user(owner)).replace("None", "Unknown") + badge)
            try:
                if post['banner']:
